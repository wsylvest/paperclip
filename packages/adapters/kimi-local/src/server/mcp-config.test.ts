...
describe("prepareMcpConfig (kimi-local)", () => {
  ...
  async function makeTmpDir(): Promise<string> {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-kimi-mcp-test-"));
  ...
  function makeInput(overrides: {
    ...
    return {
      ...
      workspaceCwd: overrides.workspaceCwd ?? "/nonexistent-kimi-workspace",
  ...
  it("returns null when workspace does not exist and seedDir is null", async () => {
    const result = await prepareMcpConfig(makeInput({ seedDir: null, workspaceCwd: "/nonexistent-kimi-workspace-xyz" }));
  ...
  it("writes only project file when seedDir is null and workspace exists", async () => {
    ...
    expect(parsed.mcpServers.paperclip.headers.Authorization).toBe("Bearer pck_test_kimi_xxx");
    // Must include the run-id header for gateway enforcement
    expect(parsed.mcpServers.paperclip.headers["X-Paperclip-Run-Id"]).toBe("run-789");
  });
  ...
  it("merges with existing mcpServers without removing user-managed entries", async () => {
    ...
    const existingConfig = {
      mcpServers: {
        foo: { type: "http", url: "https://foo.example.com" },
      },
    };
    await fs.writeFile(
  ...
  it("overwrites existing paperclip entry with new token", async () => {
    ...
    expect(parsed.mcpServers.paperclip.headers.Authorization).toBe("Bearer pck_test_kimi_xxx");
    expect(parsed.mcpServers.paperclip.url).toContain("company-123");
    // Ensure no duplicate "paperclip" key in raw JSON
