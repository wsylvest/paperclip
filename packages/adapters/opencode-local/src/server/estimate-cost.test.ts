...
describe("opencode-local estimateCost", () => {
  it("uses default model when adapterConfig has no model, calls estimateFromTokens with inputTokens in range [2000,3000]", async () => {
    ...
    expect(call.adapterType).toBe("opencode_local");
  ...
  it("parses provider/model format from adapterConfig.model and uses extracted provider and model", async () => {
    const prompt = "a".repeat(4000);
    // opencode uses "provider/model" format
    const ctx = makeCtx(prompt, { model: "openai/gpt-5-mini" });
