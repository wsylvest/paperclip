...
import { extractSkillMentionIds, isUuidLike, validateRunResultJson } from "@paperclipai/shared";
...
export function heartbeatService(db: Db, options: HeartbeatServiceOptions = {}) {
  ...
  async function executeRun(runId: string) {
    ...
    try {
      ...
      // workspace_finalize barrier (upstream PAPA-430): record a finalize
      // operation once the adapter returns (or throws) so dependents sharing
      // this executionWorkspace can wake / stay gated correctly.
      ...
      runtimeMark("run.finalized", () => ({
        runId: run.id,
        outcome,
        status: persistedRun?.status ?? status,
      }));
      if (persistedRun) {
